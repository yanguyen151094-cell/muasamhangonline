...
import { submitRegistration, getRegistrationStatus } from '../../../utils/adminAccountStorage';
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      try {
        ...
        const existing = getRegistrationStatus(form.email);
        if (existing) {
      ...
      <div className="w-full max-w-lg">
        ...
        {step === 'form' && (
          <div className="bg-gray-900 rounded-2xl p-8 border border-gray-800">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div>
                <label className="block text-gray-300 text-sm font-medium mb-1.5">
                  Lý do đăng ký / Vị trí công việc *
                </label>
                <textarea
                  value={form.reason}
                  onChange={(e) => setForm((p) => ({ ...p, reason: e.target.value.slice(0, 300) }))}
                  required
                  rows={3}
                  maxLength={300}
            ...
            <div className="mt-6 pt-5 border-t border-gray-800 text-center">
              ...
              <Link to="/admin/login" className="text-orange-400 hover:text-orange-300 text-xs font-medium transition-colors">
                Đăng nhập ngay
              </Link>
            </div>
          </div>
        ...
        {step === 'success' && (
          <div className="bg-gray-900 rounded-2xl p-10 border border-gray-800 text-center">
            ...
            <p className="text-gray-400 text-sm mb-6 max-w-sm mx-auto">
              Yêu cầu đăng ký của <span className="text-white font-medium">{form.name}</span> đã được ghi nhận.
              Admin Cấp 1 sẽ xem xét và phê duyệt trong thời gian sớm nhất.
            </p>
            ...
            <div className="space-y-2">
              <p className="text-gray-500 text-xs">Sau khi được duyệt, bạn có thể đăng nhập bình thường.</p>
              <Link
                to="/admin/login"
                className="block w-full text-center bg-gray-800 hover:bg-gray-700 text-gray-300 font-medium py-3 rounded-lg transition-colors text-sm whitespace-nowrap"
              >
                Về trang đăng nhập
              </Link>
            </div>
        ...
        {step === 'already' && existingStatus && (
          <div className="bg-gray-900 rounded-2xl p-10 border border-gray-800 text-center">
            ...
            <p className={`text-sm font-medium mb-2 ${statusLabel[existingStatus]?.color}`}>
              {statusLabel[existingStatus]?.text}
            </p>
            <p className="text-gray-500 text-xs mb-6">
              {existingStatus === 'rejected' ? 'Vui lòng liên hệ Admin Cấp 1 để biết thêm thông tin.' : 'Kiểm tra lại tình trạng bằng cách liên hệ Admin Cấp 1.'}
            </p>
            <Link
              to="/admin/login"
              className="block w-full text-center bg-gray-800 hover:bg-gray-700 text-gray-300 font-medium py-3 rounded-lg transition-colors text-sm whitespace-nowrap"
