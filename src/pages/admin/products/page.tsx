...
import { coupons } from '../../../mocks/adminData';
...
export default function AdminProductsPage() {
  ...
  const [form, setForm] = useState({ name: '', sku: '', category: 'Điện Thoại', price: '', original_price: '', stock: '', status: 'active' });
  ...
  const openAdd = () => {
    setEditProduct(null);
    setForm({ name: '', sku: '', category: 'Điện Thoại', price: '', original_price: '', stock: '', status: 'active' });
    setShowModal(true);
  };

  const openEdit = (p: Product) => {
    setEditProduct(p);
    setForm({ name: p.name, sku: p.sku ?? '', category: p.category, price: String(p.price), original_price: String(p.original_price), stock: String(p.stock), status: p.status });
    setShowModal(true);
  };
  ...
  const handleSave = async () => {
    ...
    try {
      if (editProduct) {
        ...
        await supabase.from('products').update(updateData).eq('id', editProduct.id);
        ...
        <>
          ...
          {!canDelete && (
            <div className="mb-4 flex items-center gap-2 px-4 py-3 bg-orange-50 border border-orange-200 rounded-lg text-orange-700 text-xs">
              ...
              <span>Admin Cấp 2: Có thể thêm/sửa giá, mô tả, hình ảnh, khuyến mãi. Không xóa sản phẩm, không chỉnh SKU/danh mục.</span>
            </div>
          )}
          ...
          <div className="bg-white rounded-xl border border-gray-100 overflow-hidden">
            {loadingProducts ? (
              <div className="text-center py-16">
                ...
                <p className="text-gray-400 text-sm mt-3">Đang tải sản phẩm...</p>
              </div>
            ) : (
                    ...
                    <tr key={p.id} className="hover:bg-gray-50 transition-colors">
                        ...
                        <div className="flex items-center gap-3">
                          {p.image ? (
                            ...
                            <div className="w-10 h-10 bg-gray-100 rounded-lg flex items-center justify-center flex-shrink-0">
                              <i className="ri-image-line text-gray-400 text-base"></i>
                            </div>
                          )}
                      ...
                      <td className="px-4 py-3 text-right">
                        ...
                        {p.original_price > p.price && (
                          <div className="text-xs text-gray-400 line-through">{fmt(p.original_price)}₫</div>
                        )}
                      </td>
                      <td className="px-4 py-3 text-right">
                        <span className={`text-sm font-medium ${p.stock === 0 ? 'text-red-500' : p.stock < 20 ? 'text-orange-500' : 'text-gray-900'}`}>{p.stock}</span>
                      </td>
                      ...
                      <td className="px-4 py-3 text-center">
                        <div className="flex items-center justify-center gap-1">
                          {can('products_edit') && (
                            <button onClick={() => openEdit(p)} className="w-7 h-7 flex items-center justify-center text-gray-400 hover:text-green-600 hover:bg-green-50 rounded-md transition-colors cursor-pointer" title="Chỉnh sửa">
                              <i className="ri-edit-line text-sm"></i>
                            </button>
                          )}
                          {canDelete ? (
                            <button onClick={() => setDeleteConfirm(p.id)} className="w-7 h-7 flex items-center justify-center text-gray-400 hover:text-red-600 hover:bg-red-50 rounded-md transition-colors cursor-pointer" title="Xóa">
            ...
            {!loadingProducts && filtered.length === 0 && (
              <div className="text-center py-12 text-gray-400">
                ...
                <p className="text-xs mt-1">Bấm &quot;Thêm Sản Phẩm&quot; để bắt đầu</p>
              </div>
            )}
          </div>
        </>
            ...
            <tbody className="divide-y divide-gray-50">
              {coupons.map((c) => (
                <tr key={c.id} className="hover:bg-gray-50 transition-colors">
                  ...
                  <td className="px-4 py-3 text-right">
                    <span className="text-xs text-gray-700">{c.used}<span className="text-gray-400">/{c.limit}</span></span>
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-xl w-full max-w-lg">
            ...
            <div className="px-6 py-5 space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input value={form.sku} onChange={(e) => setForm((p) => ({ ...p, sku: e.target.value }))} disabled={!canEditSKU}
                ...
                <div>
                  ...
                  <select value={form.category} onChange={(e) => setForm((p) => ({ ...p, category: e.target.value }))} disabled={!canEditCategory}
              ...
              <div className="grid grid-cols-3 gap-4">
                <div>
                  ...
                  <input type="number" value={form.price} onChange={(e) => setForm((p) => ({ ...p, price: e.target.value }))}
                    className="w-full border border-gray-200 rounded-lg px-3 py-2.5 text-sm focus:outline-none focus:border-red-400" placeholder="0" />
                </div>
                <div>
            ...
            <div className="flex gap-3 px-6 py-4 border-t border-gray-100">
              <button onClick={() => setShowModal(false)} className="flex-1 border border-gray-200 rounded-lg py-2.5 text-sm text-gray-600 hover:bg-gray-50 cursor-pointer whitespace-nowrap">Hủy</button>
              <button onClick={handleSave} disabled={saving}
                className="flex-1 bg-red-600 hover:bg-red-700 disabled:opacity-60 text-white rounded-lg py-2.5 text-sm font-medium cursor-pointer whitespace-nowrap">
                {saving ? <span className="flex items-center justify-center gap-2"><i className="ri-loader-4-line animate-spin"></i>Đang lưu...</span> : (editProduct ? 'Cập Nhật' : 'Thêm Mới')}
              </button>
            </div>
          </div>
        </div>
